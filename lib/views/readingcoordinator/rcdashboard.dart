...
class _RCDashboardState extends State<RCDashboard> {
  ...
  Widget _buildStatCards(List<SchoolYear> allYears) {
    // Stream-based total students across ALL school years
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, studentSnap) {
        final totalStudents = studentSnap.data?.docs.length ?? 0;

        return StreamBuilder<QuerySnapshot>(
          stream: _firestore.collection('schoolyears').snapshots(),
          builder: (context, sySnap) {
            final totalSchoolYears = sySnap.data?.docs.length ?? 0;

            // Compute frustration and independent from current range
            int frustrationCount = 0;
            int independentCount = 0;
            for (final y in _selectedRangeYears) {
              final counts = _rangeCounts[y.id] ?? {};
              frustrationCount += counts['Frustration'] ?? 0;
              independentCount += counts['Independent'] ?? 0;
            }
  ...
  Widget _buildGenderWinnersWidget(String gender) {
    ...
    if (_selectedRangeYears.length >= 2) {
      ...
      final firstTotal = (firstCounts['Frustration']?[gender] ?? 0) +
          (firstCounts['Instructional']?[gender] ?? 0) +
          (firstCounts['Independent']?[gender] ?? 0);
      final lastTotal = (lastCounts['Frustration']?[gender] ?? 0) +
          (lastCounts['Instructional']?[gender] ?? 0) +
          (lastCounts['Independent']?[gender] ?? 0);
      final firstIndP = _percent(firstCounts['Independent']?[gender] ?? 0, firstTotal);
      final lastIndP = _percent(lastCounts['Independent']?[gender] ?? 0, lastTotal);
      final avgInd = _selectedRangeYears.map((y) {
        final c = _genderCounts[y.id] ?? {};
        final total = (c['Frustration']?[gender] ?? 0) +
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _buildChartFromCache(gender == 'Male' ? _cachedMaleUrl : _cachedFemaleUrl, 280),
          const SizedBox(height: 12),
  ...
  Widget _buildChartFromCache(String? url, double height) {
    ...
    return Image.network(
      ...
      errorBuilder: (_, __, ___) => SizedBox(
        height: height,
        child: const Center(
          child: Text('Unable to load chart', style: TextStyle(color: AppTheme.textSecondaryColor)),
        ),
      ),
    );
  }
              ...
              children: [
                Row(
                  children: [
                    ...
                    const Icon(Icons.chevron_right, size: 13, color: AppTheme.textSecondaryColor),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      drawer: isDesktop ? null : Drawer(child: RCSidebar(activeRoute: RCRoute.dashboard)),
      body: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
