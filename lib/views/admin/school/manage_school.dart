...
import 'package:ireader_web/views/admin/school/compare_school.dart';
...
class ManageSchoolScreen extends StatefulWidget {
  ...
  const ManageSchoolScreen({super.key, required this.division});
...
class _ManageSchoolScreenState extends State<ManageSchoolScreen> {
  ...
  Stream<QuerySnapshot> _fetchSchools() => _firestore
  ...
  void _openAdd() {
    showDialog(
      ...
      builder: (_) => AddSchoolDialog(division: widget.division),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // drawer: isDesktop
      //     ? null
      //     : Drawer(child: AdminSidebar(activeRoute: AdminRoute.schools)),
      body: Row(
  ...
  Widget _buildRow(School school) {
    ...
    final avatarColor =
        avatarColors[school.name.isNotEmpty
            ? school.name.codeUnitAt(0) % avatarColors.length
            : 0];
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 3,
            child: Text(
              _divisionName(school.divisionid),
              style: const TextStyle(
                fontSize: 13,
                color: AppTheme.textSecondaryColor,
              ),
              ...
              children: [
                InkWell(
                  ...
                  onTap: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => ManageSchoolyearScreen(
                  ...
                  onChanged: (val) async {
                    final confirmed = await showDialog<bool>(
                      ...
                      builder: (_) => AlertDialog(
                        ...
                        title: Text(
                          val ? 'Activate School' : 'Deactivate School',
                        ),
                        content: Text(
                          val
                              ? 'Set "${school.name}" as active?'
                              : 'Set "${school.name}" as inactive?',
                        ),
                        actions: [
                          TextButton(
