...
class _ManageTeacherScreenState extends State<ManageTeacherScreen> {
  ...
  void _openAddTeacher() {
    ...
    if (isMobile) {
      ...
      showDialog(
        ...
        builder: (_) => const AddTeacherDialog(teacher: null),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      drawer: isDesktop ? null : Drawer(child: AdminSidebar(activeRoute: AdminRoute.teachers)),
      body: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          if (isDesktop) const AdminSidebar(activeRoute: AdminRoute.teachers),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                AdminTopHeader(
                  ...
                  pageSubtitle: 'Teacher accounts and section assignments',
                ),
                Expanded(
                  child: StreamBuilder<QuerySnapshot>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (_selectedFilter != 'All') {
                        teachers = teachers.where((t) => t.status == _selectedFilter).toList();
                      }

                      final query = _searchController.text.trim().toLowerCase();
                      if (query.isNotEmpty) {
                        teachers = teachers.where((t) {
                          final name = '${t.firstname} ${t.middlename ?? ''} ${t.lastname}'.toLowerCase();
                          return name.contains(query) || t.email.toLowerCase().contains(query);
                        }).toList();
                      }
                      ...
                      return Column(
                        children: [
                          _buildToolbar(),
                          Expanded(child: _buildTable(teachers)),
                        ],
                      );
                    },
                  ),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const Text('No teachers found', style: TextStyle(color: AppTheme.textSecondaryColor, fontSize: 15)),
          ],
        ),
      );
    }
        ...
        children: [
          ...
          Expanded(
            child: ListView.separated(
              ...
              itemBuilder: (context, index) => _buildRow(teachers[index]),
            ),
          ),
        ],
      ),
  ...
  Widget _buildRow(Teacher teacher) {
    ...
    final fullName = '${teacher.firstname} ${teacher.middlename != null && teacher.middlename!.isNotEmpty ? "${teacher.middlename} " : ""}${teacher.lastname}'.trim();
    final initial = teacher.firstname.isNotEmpty ? teacher.firstname[0].toUpperCase() : 'T';
              ...
              children: [
                InkWell(
                  ...
                  onTap: () {
                    final isMobile = MediaQuery.of(context).size.width <= 768;
                    if (isMobile) {
                      Navigator.push(context, MaterialPageRoute(builder: (_) => AddTeacherScreen(teacher: teacher)));
                    } else {
                      showDialog(
                        context: context,
                        barrierDismissible: false,
                        builder: (_) => AddTeacherDialog(teacher: teacher),
                      );
                    }
                  },
                  child: Container(
                  ...
                  onChanged: (val) async {
                    ...
                    final confirmed = await showDialog<bool>(
                      ...
                      builder: (_) => AlertDialog(
                        ...
                        title: Text(val ? 'Activate Teacher' : 'Deactivate Teacher'),
                        content: Text(
                          val
                              ? 'Set this teacher as active?'
                              : 'Set this teacher as inactive?',
                        ),
                        actions: [
                    ...
                    if (confirmed ?? false) {
                      _firestore.collection('teachers').doc(teacher.id).update({'status': newStatus});
                    }
                  },
                ),
              ],
